...
const call = async (tool, args) => { const r = await send('tools/call', { name: tool, arguments: args }); const t = r.result; if (!t || t.isError) return { error: (t && t.content && t.content[0].text) || JSON.stringify(r.error) }; if (t.structuredContent) return t.structuredContent; try { return JSON.parse(t.content[0].text); } catch { return { text: t.content[0].text }; } };
