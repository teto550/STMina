...
Moved from the plain modal to `src/react/screens/EditServant.tsx` (registry `edit-servant`; opened from `openEditDeaconProfile` in `features/servants/servants.ts`, which prepares the data and applies the result). The old modal HTML (`#edit-deacon-profile-modal`) and its functions were removed from `index.html`/`servants.ts`. New shared pieces: `src/core/servant-rename.ts` (`renameServant`, also used by the older "rename servant" modal in `features/servants/deacons-tab.ts`, which now also updates attendance records and every login), `src/core/universities.ts` (list moved out of `servants.ts`), `window.openReactScreen(name, container?, props?)` accepts props (`react/bootstrap.ts`, `react/screens/registry.ts`). A single-word name is allowed. Test: `src/react/__tests__/EditServant.test.tsx`.
