...
// بيتحقق إن اللي طالب إرسال إشعار الفقرة فعلاً مسؤول عن الفصل ده (أدمن أو مسؤول سنة/مرحلة) — نفس منطق isGradeManagerOf في الموقع
function canManageGrade(user, grade) {
  if (!user) return false;
  if (user.role === 'admin') return true;
  if (user.isLead && user.grade === grade) return true;
  if (user.isPhaseLead) {
    const phase = Array.isArray(user.phaseGrades)
      ? user.phaseGrades
      : String(user.phaseGrades || '').split(/[;,]/).map((x) => x.trim());
    if (phase.includes(grade)) return true;
  }
  return false;
}
...
function formatPartDate(dateStr) {
  ...
  if (!y || !m || !d) return '';
  return `${PART_WEEKDAY_AR[new Date(y, m - 1, d).getDay()]}، ${d}/${m}`;
}

// ===== /new-part: إشعار Push للخادم اللي اتوزعت عليه فقرة =====
async function handleNewPart(request, env, cors) {
  try {
    const sa = JSON.parse(env.SERVICE_ACCOUNT);
    const pid = sa.project_id;

    const bearer = (request.headers.get('Authorization') || '').replace(/^Bearer\s+/i, '');
    const { sub: senderUid } = await verifyIdToken(bearer, pid);

    let body;
    try { body = await request.json(); } catch (e) { return json({ error: 'bad body' }, 400, cors); }
    const deaconName = String(body.deaconName || '').trim();
    const grade = String(body.grade || '').trim();
    const type = String(body.type || '').trim();
    const title = String(body.title || '').trim();
    const date = String(body.date || '').trim();
    if (!deaconName || !grade || !type || !title) return json({ error: 'missing fields' }, 400, cors);

    const at = await getAccessToken(sa);
    const authH = { Authorization: `Bearer ${at}` };
    const jsonH = { ...authH, 'Content-Type': 'application/json' };

    // 1) تأكيد إن اللي بعت الطلب فعلاً مسؤول عن الفصل ده (أدمن أو مسؤول سنة/مرحلة)
    const sr = await fetch(`${fsBase(pid)}/users/${senderUid}`, { headers: authH });
    if (!sr.ok) return json({ error: 'no sender doc' }, 404, cors);
    const sender = fvFields((await sr.json()).fields);
    if (!canManageGrade(sender, grade)) return json({ error: 'forbidden' }, 403, cors);

    // 2) دور على الخادم المستهدف بالاسم — لازم يكون معتمد (status=approved) وعنده fcmTokens
        ...
        structuredQuery: {
          ...
          where: {
            compositeFilter: {
              op: 'AND',
              filters: [
                { fieldFilter: { field: { fieldPath: 'name' }, op: 'EQUAL', value: { stringValue: deaconName } } },
    ...
    if (!tokens.length) return json({ ok: true, sent: 0, total: 0, skipped: 'no tokens' }, 200, cors);

    // 3) ابعت الـ Push
    const pushTitle = '📋 اتوزعت عليك فقرة';
    const dateLabel = formatPartDate(date);
    const pushBody = `${type} — ${title}${dateLabel ? ' · ' + dateLabel : ''}`;
    const results = await Promise.allSettled(tokens.map((t) =>
      fetch(`https://fcm.googleapis.com/v1/projects/${pid}/messages:send`, {
        ...
        body: JSON.stringify({
          message: {
            token: t,
            data: { title: pushTitle, body: pushBody, url: env.APP_URL || '', tag: `new-part-${senderUid}-${Date.now()}` },
...
export default {
  async fetch(request, env) {
    ...
    if (request.method !== 'POST' || (url.pathname !== '/new-deacon' && url.pathname !== '/new-part')) {
      return json({ error: 'not found' }, 404, cors);
    }
    if (allowed && request.headers.get('Origin') !== allowed) return json({ error: 'forbidden' }, 403, cors);

    if (url.pathname === '/new-part') return handleNewPart(request, env, cors);
